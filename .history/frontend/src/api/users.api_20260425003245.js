...
export const usersApi = {
  ...
  // Profile lookups by publicId
  getAgentByPublicId:  (pid) => client.get(`/agents/by-public-id/${pid}`).then(r => r.data).catch(() => null),
  getOwnerByPublicId:  (pid) => client.get(`/owners/by-public-id/${pid}`).then(r => r.data).catch(() => null),
  getClientByPublicId: (pid) => client.get(`/clients/by-public-id/${pid}`).then(r => r.data).catch(() => null),
}

export const clientsApi = {
  getAll:        ()           => client.get("/clients").then(r => r.data),
  getByCompany:  (companyId) => client.get("/clients/by-company", { params: { companyId } }).then(r => r.data),
}
