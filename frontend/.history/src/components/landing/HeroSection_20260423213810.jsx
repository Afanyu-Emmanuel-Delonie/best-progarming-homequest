...
export default function HeroSection() {
  ...
  return (
    <section style={{ position: "relative", minHeight: "100vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      {/* Stats strip */}
      <div style={{ position: "relative", zIndex: 2, display: "flex", flexWrap: "wrap", background: "rgba(0,0,0,0.55)", backdropFilter: "blur(10px)", borderTop: "1px solid rgba(255,255,255,0.1)" }}>
        {STATS.map((s, i) => (
          <div key={s.label} style={{ flex: "1 1 140px", padding: "1.25rem 1.5rem", borderRight: i < STATS.length - 1 ? "1px solid rgba(255,255,255,0.1)" : "none", textAlign: "center" }}>
            <p style={{ margin: 0, fontWeight: 800, fontSize: "1.3rem", color: "#fff" }}>{s.value}</p>
            <p style={{ margin: "3px 0 0", fontSize: "0.68rem", color: "rgba(255,255,255,0.5)", textTransform: "uppercase", letterSpacing: "0.07em" }}>{s.label}</p>
          </div>
        ))}
      </div>
    </section>
