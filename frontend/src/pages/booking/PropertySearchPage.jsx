...
import { BedDouble, Bath, Maximize2, MapPin, Search, Heart, X, Loader2 } from "lucide-react"
...
export default function PropertiesPage() {
  ...
  const filtered = useMemo(() => properties.filter(p => {
    ...
    return true
  }), [properties, query, type, maxPrice])
  ...
  useEffect(() => {
    ...
    cards.forEach((c, i) => {
      ...
      setTimeout(() => { c.style.transition = "opacity 0.4s ease, transform 0.4s ease"; c.style.opacity = "1"; c.style.transform = "translateY(0)" }, i * 60)
    })
  }, [filtered])
  ...
  const clearFilters = () => { setType("All"); setMaxPrice(""); setQuery("") }
  const activeFilters = (type !== "All" ? 1 : 0) + (maxPrice ? 1 : 0) + (query ? 1 : 0)

  return (
    <div style={{ paddingTop: 64, minHeight: "100vh", backgroundColor: "#FAFAFA" }}>

      <div style={{ position: "relative", height: 260, overflow: "hidden" }}>
        <img src="https://images.unsplash.com/photo-1611348586804-61bf6c080437?w=1600&q=80" alt="Kigali"
        ...
        <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "0 1.5rem" }}>
          ...
          <p style={{ margin: 0, fontSize: "0.9375rem", color: "rgba(255,255,255,0.65)" }}>
            {loading ? "Loading listings…" : `${properties.length} verified listings`}
          </p>
        </div>
      </div>
          ...
          {TYPE_TABS.map(t => {
            ...
            return (
              <button key={t} onClick={() => setType(t)} style={{
                ...
                cursor: "pointer", fontFamily: "inherit", whiteSpace: "nowrap", marginBottom: "-1px",
              }}>
      ...
      <div className="props-body" style={{ maxWidth: 1280, margin: "0 auto", padding: "2rem 1.5rem", display: "flex", gap: "2rem", alignItems: "flex-start" }}>
        ...
        <aside className="filter-sidebar" style={{ width: 260, flexShrink: 0, position: "sticky", top: 80, backgroundColor: "#fff", borderRadius: "14px", border: "1px solid var(--color-border)", padding: "1.5rem", display: "flex", flexDirection: "column", gap: "1.5rem" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            {activeFilters > 0 && (
              <button onClick={clearFilters} style={{ fontSize: "0.78rem", color: "var(--color-primary)", fontWeight: 600, background: "none", border: "none", cursor: "pointer", fontFamily: "inherit" }}>Clear all</button>
            )}
          </div>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          {loading ? (
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "5rem 0", gap: "0.75rem", color: "var(--color-text-muted)" }}>
              <Loader2 size={20} style={{ animation: "spin 1s linear infinite" }} /> Loading properties…
  ...
  return (
    ...
    >
      <div style={{ position: "relative", height: 210, overflow: "hidden", backgroundColor: "var(--color-bg-muted)" }}>
        ...
        <button onClick={e => onSave(p.id, e)}
      ...
      <div style={{ padding: "1.125rem 1.25rem 1.25rem", flex: 1, display: "flex", flexDirection: "column", gap: "0.75rem" }}>
        ...
        <div style={{ display: "flex", gap: "0.5rem", marginTop: "auto" }}>
          <button onClick={e => { e.stopPropagation(); navigate(ROUTES.PROPERTY_DETAIL.replace(":id", p.id)) }}
            style={{ flex: 1, padding: "0.55rem 0", borderRadius: "8px", border: "1px solid var(--color-border)", backgroundColor: "transparent", fontSize: "0.8125rem", fontWeight: 600, color: "var(--color-text)", cursor: "pointer", fontFamily: "inherit" }}
            onMouseEnter={e => e.currentTarget.style.backgroundColor = "var(--color-bg-muted)"}
            onMouseLeave={e => e.currentTarget.style.backgroundColor = "transparent"}
          >View Details</button>
          {p.status === "AVAILABLE" ? (
            <button onClick={e => { e.stopPropagation(); navigate(ROUTES.BOOKING_FORM.replace(":id", p.id)) }}
              style={{ flex: 1, padding: "0.55rem 0", borderRadius: "8px", border: "none", backgroundColor: "var(--color-primary)", fontSize: "0.8125rem", fontWeight: 600, color: "#fff", cursor: "pointer", fontFamily: "inherit" }}
              onMouseEnter={e => e.currentTarget.style.opacity = "0.88"}
              onMouseLeave={e => e.currentTarget.style.opacity = "1"}
            >Apply Now</button>
          ) : (
            <div style={{ flex: 1, padding: "0.55rem 0", borderRadius: "8px", backgroundColor: st.bg, fontSize: "0.8125rem", fontWeight: 600, color: st.color, textAlign: "center" }}>
              {st.label}
            </div>
          )}
        </div>
