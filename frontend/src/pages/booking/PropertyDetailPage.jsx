...
import { ArrowLeft, MapPin, BedDouble, Bath, Maximize2, Heart, Check, Loader2 } from "lucide-react"
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "1.25rem 1.5rem 4rem", display: "flex", gap: "2.5rem", alignItems: "flex-start" }} className="detail-body">
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ borderRadius: "16px", overflow: "hidden", height: 420, backgroundColor: "var(--color-bg-muted)", position: "relative" }} className="detail-main-img">
            <img src={imgs[activeImg]} alt={p.title} style={{ width: "100%", height: "100%", objectFit: "cover", transition: "opacity 0.3s" }} />
            ...
            <span style={{ position: "absolute", top: "1rem", right: "1rem", backgroundColor: "rgba(0,0,0,0.45)", backdropFilter: "blur(4px)", color: "#fff", borderRadius: "6px", padding: "0.25rem 0.7rem", fontSize: "0.75rem", fontWeight: 600 }}>{PROPERTY_TYPE_LABELS[p.type] ?? p.type}</span>
          </div>
          ...
          <div style={{ marginTop: "2rem" }}>
            ...
            <p style={{ margin: 0, fontSize: "0.9rem", color: "var(--color-text-muted)", lineHeight: 1.75 }}>{p.description ?? "No description available."}</p>
          </div>
        ...
        <aside style={{ width: 320, flexShrink: 0, position: "sticky", top: 80 }} className="detail-aside">
          <div style={{ backgroundColor: "#fff", borderRadius: "16px", border: "1px solid var(--color-border)", padding: "1.75rem", display: "flex", flexDirection: "column", gap: "1.25rem" }}>
            <div>
              <p style={{ margin: "0 0 0.25rem", fontWeight: 800, fontSize: "1.625rem", color: "var(--color-text)", letterSpacing: "-0.02em" }}>{fmtCurrencyFull(Number(p.price))}</p>
              ...
              <p style={{ margin: "0.3rem 0 0", fontSize: "0.8125rem", color: "var(--color-text-muted)", display: "flex", alignItems: "center", gap: "0.3rem" }}>
                <MapPin size={12} style={{ color: "var(--color-primary)" }} />{p.city}
              </p>
            </div>
